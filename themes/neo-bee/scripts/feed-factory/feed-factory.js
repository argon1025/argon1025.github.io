hexo.extend.helper.register('feed',(title, subtitle, categoryName, iconName, createdAt, tags, customThumbnailPath)=>{
    // load config
    ...
    <div class="flex w-full justify-center">
        <div class="flex relative flex-col w-full max-w-5xl justify-center space-y-4">
                    <div class="z-0 rounded-3xl overflow-hidden h-56 w-full hover:drop-shadow-xl transition duration-300 ease-in-out">
                        <img src=${thumbnailPath} alt="Thumbnail" class="w-full h-full object-cover"></img>
                    </div>
...
hexo.extend.helper.register('mainFeed',(title, subtitle, categoryName, iconName, createdAt, tags, customThumbnailPath)=>{
    // load config
    const config = hexo.theme.config;
    // random thumbNail
    const randomThumbnailData = config.post.thumbnail.sort(() => Math.random() - 0.5)[0];
